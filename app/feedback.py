...
POSITIVE_KEYWORDS = [
    ...
    "increíble", "maravilloso", "espectacular", "impecable", "brillante",  # ← agregar
...
def classify_feedback_with_scores(message: str, min_score: float | None = None, top_k: int | None = None) -> list[tuple[str, float]]:
    """Reotrna una lista de (categoria, score) ordenada por score de manera descendente

    - `min_score`: mínimo coseno para el match, variable opcional que permite filtrar matches por debajo de cierto umbral de similitud. Si no se especifica, se usan los umbrales definidos en `THRESHOLD_BY_INTENT` o `THRESHOLD_DEFAULT`.
    - `top_k`: cantidad de categorías retornadas, variable opcional que limita la cantidad de matches retornados. Si no se especifica, se retornan todas las categorías por encima del umbral.
    """
...
def estimate_happiness_score(message: str) -> int:
    ...
    score = 5
    if _contains_positive(normalized, POSITIVE_KEYWORDS):  # ← asegurate que sea _contains_positive
        score += 3
    if _contains_negative(normalized, NEGATIVE_KEYWORDS):
        score -= 3
    if "muy" in normalized and _contains_positive(normalized, ["bien", "bueno", "excelente", "recomend", "increíble"]):
        score += 1
...
def estimate_nps_score(message: str, happiness_score: int) -> int:
    normalized = _normalize_text(message)
    if _contains_positive(normalized, PROMOTER_KEYWORDS):   # ← was _contains_any
        return 9
    if _contains_negative(normalized, DETRACTOR_KEYWORDS):  # ← was _contains_any
        return 1
    if _contains_any(normalized, PASSIVE_KEYWORDS):         # passive needs no negation check
        return 6
...
def estimate_return_likelihood_score(message: str, happiness_score: int | None = None) -> int:
    normalized = _normalize_text(message)
    if _contains_positive(normalized, RETURN_POSITIVE_KEYWORDS):  # ← was _contains_any
        return 9
    if _contains_negative(normalized, RETURN_NEGATIVE_KEYWORDS):  # ← was _contains_any
        return 1
    if _contains_any(normalized, RETURN_PASSIVE_KEYWORDS):        # passive needs no negation check
        return 5
...
def save_feedback(
    ...
    # Get categories (and scores) from embedding classifier unless explicit categories provided
    if categories is None:
        matches = classify_feedback_with_scores(user_message)
        categories = [intent for intent, _ in matches]
        categorias_scores = [
            {"categoria": intent, "score": round(float(score), 4)} for intent, score in matches
        ]
    else:
        # normalize provided categories and keep scores empty
        categories = [cat.strip() for cat in categories if cat and isinstance(cat, str)]
        categorias_scores = []

    primary_category = category or categories[0] if categories else DEFAULT_FEEDBACK_CATEGORY
